...
begin
  ...
  loop
    ...
    if clean_metric_key = '' or metric_label = '' then
    ...
    select definition.id into existing_definition_id
    from public.analytics_metric_definitions definition
    where definition.department = target_department
      and definition.module_key = target_module
      and definition.metric_key = clean_metric_key
    limit 1;

    if existing_definition_id is not null and exists (
      select 1
      from public.analytics_metric_values saved
      where saved.metric_id = existing_definition_id
        and saved.zone = target_zone
        and saved.period_month = normalized_month
        and saved.source_type = 'manual'
    ) then
