-- CC ANALYTICS · SUSTITUCIÓN DE IMPORTACIONES DEPARTAMENTALES
-- Un nuevo archivo para el mismo módulo, zona y mes reemplaza los registros
-- importados anteriores. Los indicadores corregidos manualmente no se borran.
...
revoke all on function public.analytics_finalize_department_import(uuid,text,text,text,date)
  from public;
grant execute on function public.analytics_finalize_department_import(uuid,text,text,text,date)
  to authenticated;
