...
export function AuthShell() {
  ...
  async function importData(upload: Upload, rows: ImportedRow[]) {
    ...
    if (importError || !created)
      return (
        importError?.message || "No se pudo crear el registro de importación."
      );
...
function friendlySupabaseError(message: string) {
  ...
  if (
    ...
    return "Ese correo ya tiene una cuenta en Supabase.";
...
function mapProfile(row: HubProfile): Profile {
  ...
  const initials = name
    ...
    .toUpperCase();
  const jobProfile =
    role === "Administrador" ? "Administrador" : row.job_title?.trim() || role;
  const zone = role === "Administrador" ? "Nacional" : normalizeZone(row.zone);
  return {
    id: row.id,
    name,
    email: row.email,
...
function normalizeDepartment(
  ...
  if (!found)
    throw new Error(
      `Tu departamento (${value || "sin asignar"}) todavía no está configurado para CC ANALYTICS.`,
    );
  return found;
}
...
function LoginScreen() {
  ...
  async function reset() {
    if (!email.trim()) {
      setError("Escribe primero tu correo corporativo.");
      return;
    }
    setLoading(true);
    setError("");
    ...
    else setMessage("Te enviamos un enlace para restablecer tu contraseña.");
    setLoading(false);
  }
  return (
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-[#070709] p-5 text-white">
      ...
      <div className="relative grid w-full max-w-5xl overflow-hidden rounded-[28px] border border-white/[.08] bg-[#0d0d12]/95 shadow-2xl lg:grid-cols-[1.05fr_.95fr]">
        <section className="hidden min-h-[620px] flex-col justify-between border-r border-white/[.06] bg-gradient-to-br from-purple-950/40 via-[#101016] to-[#0b0b0f] p-12 lg:flex">
          <div className="flex items-center gap-3">
            <div className="grid h-12 w-12 place-items-center rounded-2xl border border-purple-400/30 bg-purple-500/10 text-lg font-black text-purple-300">
              CC
            </div>
            <div>
          ...
          <div>
            <span className="inline-flex items-center gap-2 rounded-full border border-purple-400/20 bg-purple-500/10 px-3 py-1.5 text-[10px] font-bold text-purple-300">
              <ShieldCheck size={13} /> ACCESO EMPRESARIAL
            </span>
            ...
            <p className="mt-5 max-w-md text-sm leading-7 text-zinc-400">
              Utiliza la misma cuenta de CC HUB. Tus módulos, datos y permisos
              se cargan automáticamente según tu perfil.
            </p>
            <div className="mt-8 grid gap-3">
              {[
                "Una sola cuenta para CC HUB y Analytics",
              "Información protegida por departamento y zona",
        ...
        <section className="flex min-h-[620px] flex-col justify-center p-7 sm:p-12">
          <div className="mb-8 lg:hidden">
            <div className="text-lg font-black tracking-[.18em]">
              CC ANALYTICS
            </div>
            <div className="text-[9px] uppercase tracking-[.25em] text-purple-400">
          ...
          <p className="mt-2 text-xs text-zinc-500">
            Ingresa con las credenciales que utilizas en CC HUB.
          </p>
          <form onSubmit={login} className="mt-8 space-y-4">
            <label className="block text-[10px] font-bold uppercase tracking-wider text-zinc-600">
              ...
              <div className="mt-2 flex items-center gap-3 rounded-xl border border-white/[.08] bg-white/[.025] px-3 focus-within:border-purple-400/50">
                ...
                <input
                  ...
                  required
                  autoComplete="email"
                  placeholder="nombre@cablecolor.hn"
                  className="w-full bg-transparent py-3.5 text-sm outline-none placeholder:text-zinc-700"
                />
              </div>
            </label>
            <label className="block text-[10px] font-bold uppercase tracking-wider text-zinc-600">
              Contraseña
              <div className="mt-2 flex items-center gap-3 rounded-xl border border-white/[.08] bg-white/[.025] px-3 focus-within:border-purple-400/50">
                <LockKeyhole size={16} className="text-zinc-600" />
                <input
                  value={password}
            ...
            {error && (
              <p className="rounded-xl border border-rose-500/15 bg-rose-500/[.06] p-3 text-xs text-rose-300">
                {error}
              </p>
            )}
            {message && (
              <p className="rounded-xl border border-emerald-500/15 bg-emerald-500/[.06] p-3 text-xs text-emerald-300">
                {message}
              </p>
            )}
            <button
              disabled={loading}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-purple-600 to-fuchsia-600 p-3.5 text-xs font-black shadow-[0_0_30px_rgba(168,85,247,.2)] disabled:opacity-50"
            >
          ...
          <button
            onClick={reset}
            disabled={loading}
            className="mt-5 text-xs font-semibold text-purple-300 hover:text-purple-200"
          >
            ¿Olvidaste tu contraseña?
          </button>
          <div className="mt-8 border-t border-white/[.06] pt-6 text-center text-[10px] leading-5 text-zinc-600">
            Tu cargo, departamento y zona son administrados desde CC HUB.
          </div>
        </section>
      </div>
    </main>
    ...
    <main className="grid min-h-screen place-items-center bg-[#070709] p-5 text-white">
      <form onSubmit={save} className="glass w-full max-w-md rounded-3xl p-8">
        ...
        <input
          ...
          minLength={8}
          required
          className="mt-6 w-full rounded-xl border border-white/10 bg-white/[.03] p-3 text-sm outline-none focus:border-purple-400/50"
        />
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#070709] text-white">
      <div className="text-center">
        <div className="mx-auto grid h-14 w-14 animate-pulse place-items-center rounded-2xl bg-purple-500/10 font-black text-purple-300">
          CC
        </div>
        <p className="mt-4 text-xs font-bold uppercase tracking-[.2em] text-zinc-500">
          Cargando tu perfil...
        </p>
      </div>
    </main>
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#070709] p-5 text-white">
      <div className="glass w-full max-w-md rounded-3xl p-8 text-center">
        <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-rose-500/10 text-rose-400">
          <LockKeyhole />
        </div>
