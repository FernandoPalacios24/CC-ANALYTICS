...
function Card({
  ...
  return (
    <section
      className={`rounded-2xl border border-white/[.07] bg-white/[.025] ${className}`}
    >
      {children}
    </section>
  );
...
function saleUnits(value: number | null | undefined) {
...
function previewKey(sale: CanonicalSale) {
  return normalizeImportLabel(sale.sellerName);
}
...
export function SalesDataHubV2({
  ...
  const [importStart, setImportStart] = useState(monthStart());
  const [importEnd, setImportEnd] = useState(today());
  ...
  useEffect(() => {
    if (!supervisorId && supervisors[0]?.id) {
      setSupervisorId(supervisors[0].id);
    }
  }, [supervisorId, supervisors]);

  const selectedSupervisor =
    supervisors.find((item) => item.id === supervisorId) || null;
  const selectedSellers = useMemo(
    () =>
      sellers.filter(
        (seller) =>
          !supervisorId || seller.supervisor_profile_id === supervisorId,
      ),
  ...
  async function refresh() {
    ...
    const [sellerResult, salesResult, announcedResult, importResult] =
      await Promise.all([
        supabase
        ...
        supabase
          ...
          .select(
            "id,seller_id,seller_name,seller_code,supervisor_profile_id,sale_date,amount_billed,sale_units,source_type",
          )
          .gte("sale_date", start)
          .lt("sale_date", end)
        ...
        supabase
          ...
          .select(
            "id,seller_id,seller_name,seller_code,supervisor_profile_id,announced_at,expected_post_date,amount_announced,sale_units,city,service,contract_service,notes,status",
          )
        ...
        supabase
          ...
          .select(
            "id,file_name,module,row_count,supervisor_profile_id,period_start,period_end,snapshot_as_of,import_mode,replaced_rows,superseded_by,created_at",
          )
  ...
  async function saveSeller(event: React.FormEvent) {
    ...
    if (error) {
      ...
      setNotice(`${sellerName} fue agregado al equipo.`);
      setSellerName("");
      setSellerCode("");
      setHireDate(today());
  ...
  async function retireSeller(seller: SellerRecord) {
    if (
      !window.confirm(
        `¿Retirar a ${seller.full_name}? Sus ventas e historial permanecerán guardados.`,
      )
    ) {
      return;
    }
    ...
    if (error) {
      ...
      setNotice(
        result?.deferred
          ? `La salida quedó programada para ${formatDate(
              result.effective_date || null,
            )} porque tiene ventas en el mes.`
          : `${seller.full_name} fue retirado sin eliminar su historial.`,
      );
      await refresh();
    }
    setSaving(false);
  ...
  async function restoreSeller(seller: SellerRecord) {
    ...
    if (error) {
      ...
      setNotice(`${seller.full_name} fue reactivado.`);
      await refresh();
    }
    setSaving(false);
  }
  ...
  async function parseFile(file: File) {
    if (!importStart || !importEnd || importStart > importEnd) {
      setNotice("ERROR: Define un rango de fechas válido antes de subir el archivo.");
      return;
    }
    ...
    try {
      ...
      if (file.name.toLowerCase().endsWith(".csv")) {
        ...
        for (const sheetName of book.SheetNames) {
          ...
          const bestScore = best
            ? best.rows.length + best.confidence * 100
            : -1;
          if (score > bestScore) {
            best = analyzed;
            bestSheet = sheetName;
      ...
      if (!best?.rows.length) {
        throw new Error("No se detectaron filas con vendedores.");
      }

      const normalizedRows = best.rows.map((sale) =>
        best?.map.saleDate ? sale : { ...sale, saleDate: importEnd },
      );
      const withinRange = normalizedRows.filter(
  ...
  const unresolvedNames = Array.from(
    new Map(unresolved.map((row) => [row.key, row])).values(),
  );
  const totalImportUnits = importPreview.reduce(
  ...
  async function confirmImport() {
    ...
    const { data: created, error: importError } = await supabase
      ...
      .insert({
        ...
        module,
        row_count: totalImportUnits,
        supervisor_profile_id: selectedSupervisor.id,
        period_start: importStart,
        period_end: importEnd,
    ...
    if (importError || !created) {
      setNotice(
        `ERROR: ${
          importError?.message || "No se pudo registrar la importación."
        }`,
      );
      setSaving(false);
      return;
    ...
    for (const batch of chunk(importPreview)) {
      ...
      if (error) {
        setNotice(`ERROR: La carga de respaldo quedó incompleta: ${error.message}`);
        setSaving(false);
        return;
      }
    }
    ...
    if (importKind === "posted") {
      for (const batch of chunk(importPreview)) {
        const { error } = await supabase.from("analytics_sales").insert(
          batch.map(({ sale, match, units, totalAmount }) => ({
            ...
            zone: match.seller?.zone || selectedSupervisor.zone,
            seller_id: match.seller?.id || null,
            seller_profile_id: null,
            supervisor_profile_id:
              match.seller?.supervisor_profile_id || selectedSupervisor.id,
            seller_code: match.seller?.seller_code || sale.sellerCode,
      ...
      for (const batch of chunk(importPreview)) {
        ...
        if (error) {
          setNotice(
            `ERROR: No se guardaron las ventas anunciadas: ${error.message}`,
          );
          setSaving(false);
          return;
        }
    ...
    const { data: finalized, error: finalizeError } = await supabase.rpc(
      ...
      {
        current_import_id: created.id,
        target_supervisor_id: selectedSupervisor.id,
        target_start: importStart,
        target_end: importEnd,
        target_module: module,
        target_mode: importMode,
      },
    );

    if (finalizeError) {
      setNotice(
        `ERROR: Los datos nuevos se guardaron, pero no se completó la sustitución: ${finalizeError.message}`,
      );
      setSaving(false);
      return;
    }
    ...
    const result = finalized as {
      removed_sales_rows?: number;
      removed_announced_rows?: number;
    } | null;
    const removed =
      Number(result?.removed_sales_rows || 0) +
      Number(result?.removed_announced_rows || 0);

    setNotice(
      `${totalImportUnits.toLocaleString("es-HN")} ventas actualizadas del ${formatDate(
        importStart,
      )} al ${formatDate(importEnd)}. ${
        importMode === "replace"
          ? `Se sustituyeron ${removed} filas importadas anteriores; las ventas manuales se conservaron.`
          : "La carga se agregó sin sustituir datos anteriores."
      } ${
        unresolved.length
          ? `${unresolved.length} filas quedaron por nombre original.`
          : "Todos los vendedores quedaron vinculados."
      }`,
    );
    setImportRows([]);
    setImportFile("");
  ...
  async function saveManualSale(event: React.FormEvent) {
    ...
    const supervisor = profiles.find(
      (item) => item.id === seller.supervisor_profile_id,
    );
    const { error } = await supabase.from("analytics_sales").insert({
  ...
  async function updateAnnounced(
    id: number,
    ...
    if (error) setNotice(`ERROR: ${error.message}`);
      ...
      setNotice(
        status === "posteada"
          ? "Venta anunciada marcada como posteada."
          : "Venta anunciada cancelada.",
      );
      await refresh();
    }
    setSaving(false);
  ...
  const filteredSales = useMemo(
    () =>
      sales.filter(
        (sale) =>
          !supervisorId || sale.supervisor_profile_id === supervisorId,
      ),
    [sales, supervisorId],
  );
  ...
  const filteredImports = useMemo(
    () =>
      imports.filter(
        (item) =>
          !supervisorId || item.supervisor_profile_id === supervisorId,
      ),
    [imports, supervisorId],
  );

  const saleAssignments = useMemo(() => {
    ...
    return result;
  }, [filteredSales, selectedSellers]);

  const announcedAssignments = useMemo(() => {
    const result = new Map<number, string | null>();
    filteredAnnounced.forEach((sale) => {
      if (sale.seller_id) {
        result.set(sale.id, sale.seller_id);
      } else {
    ...
    return result;
  }, [filteredAnnounced, selectedSellers]);
        ...
        .map((seller) => {
          ...
          const sellerAnnounced = filteredAnnounced.filter(
            (sale) =>
              ...
              announcedAssignments.get(sale.id) === seller.id,
          );
          return {
            seller,
            posted: sellerSales.reduce(
              (sum, sale) => sum + saleUnits(sale.sale_units),
              0,
            ),
            announced: sellerAnnounced
              .filter((sale) => sale.status === "anunciada")
              .reduce(
                (sum, sale) => sum + saleUnits(sale.sale_units),
                0,
              ),
            probationSales: sellerSales
              .filter((sale) => sellerProbationAt(seller, sale.sale_date))
              .reduce(
                (sum, sale) => sum + saleUnits(sale.sale_units),
                0,
              ),
            totalAmount: sellerSales.reduce(
  ...
  const monthPosted = filteredSales.reduce(
    (sum, sale) => sum + saleUnits(sale.sale_units),
    0,
  );
  const monthAnnounced = filteredAnnounced
    .filter((sale) => sale.status === "anunciada")
    .reduce((sum, sale) => sum + saleUnits(sale.sale_units), 0);
    ...
    <div className="space-y-4 text-white">
        ...
        <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black">
              Equipos, cargas acumuladas y sustitución por fecha
            </h2>
            <p className="mt-2 max-w-4xl text-xs leading-5 text-zinc-500">
              El algoritmo reconoce variantes de nombres, conserva vendedores
              retirados y permite reemplazar únicamente las ventas importadas de
              un rango. Las ventas manuales nunca se eliminan durante una
              actualización.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <select
              ...
              disabled={
                profile.role === "Supervisor" || supervisors.length <= 1
              }
            ...
            >
              {!supervisors.length && (
                <option value="">Sin supervisor disponible</option>
              )}
        ...
        {tabs.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex shrink-0 items-center gap-2 rounded-xl px-4 py-3 text-xs font-bold transition ${
                tab === item.id
        ...
        <>
            ...
            <div className="grid gap-4 xl:grid-cols-[380px_1fr]">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-[10px] text-zinc-600">
                      El historial queda unido al registro, no al listado activo.
                    </p>
                  </div>
                </div>
                {canManageTeam ? (
                  <form onSubmit={saveSeller} className="mt-5 space-y-4">
                    <label className="block text-[10px] font-black uppercase tracking-wider text-zinc-600">
                      ...
                      <input
                        ...
                        className="mt-2 w-full rounded-xl border border-white/[.08] bg-[#111116] p-3 text-xs outline-none"
                      />
                    </label>
                    <label className="block text-[10px] font-black uppercase tracking-wider text-zinc-600">
                      Código · opcional
                    ...
                    <label className="block text-[10px] font-black uppercase tracking-wider text-zinc-600">
                      ...
                      <input
                        ...
                        value={probationDays}
                        onChange={(event) =>
                          setProbationDays(Number(event.target.value))
                        }
                        className="mt-2 w-full rounded-xl border border-white/[.08] bg-[#111116] p-3 text-xs"
                      />
                    </label>
                  ...
                  <p className="mt-5 text-xs text-amber-300">
                    Tu perfil no puede modificar equipos.
                  </p>
                )}
              </Card>
              ...
              <Card className="overflow-hidden">
                <div className="border-b border-white/[.06] p-5">
                  ...
                  <p className="mt-1 text-[10px] text-zinc-600">
                    Retirar no borra ventas ni reportes.
                  </p>
                </div>
                        ...
                        >
                          ...
                          <td>
                            {seller.is_on_probation ? (
                              <span className="text-cyan-300">EN PRUEBA</span>
                            ) : (
                          ...
                          <td className="pr-5 text-right">
                            {canManageTeam &&
                              seller.effective_status === "activo" && (
                                <button
                                  disabled={saving}
                                  onClick={() => void retireSeller(seller)}
                            ...
                            {canManageTeam &&
                              seller.effective_status !== "activo" && (
                                <button
                                  disabled={saving}
                                  onClick={() => void restoreSeller(seller)}
            ...
            <div className="space-y-4">
                ...
                <div className="flex flex-col gap-5">
                  <div>
                    <h3 className="font-black">
                      Actualización acumulada por rango
                    </h3>
                    <p className="mt-2 max-w-4xl text-xs leading-5 text-zinc-500">
                      Selecciona el período que representa el archivo. En modo
                      sustituir, una carga del 1 al 30 reemplaza la carga
                      anterior del 1 al 28 para este supervisor, sin sumar ambas.
                    </p>
                  </div>
                  ...
                  <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">
                    <label className="text-[10px] font-black uppercase tracking-wider text-zinc-600">
                      ...
                      <select
                        value={importKind}
                        onChange={(event) =>
                          setImportKind(event.target.value as ImportKind)
                        }
                    ...
                    <label className="text-[10px] font-black uppercase tracking-wider text-zinc-600">
                      ...
                      >
                        ...
                        <option value="append">Agregar sin sustituir</option>
                      </select>
                    </label>
                    <div className="rounded-xl border border-purple-400/15 bg-purple-500/[.05] p-3 text-[10px] leading-5 text-zinc-400">
                      <b className="text-purple-300">Protección:</b> solo se
                      sustituyen cargas importadas. Los registros manuales
                      permanecen.
                    </div>
                  </div>
                  ...
                  <div
                    ...
                    className="grid min-h-52 cursor-pointer place-items-center rounded-2xl border border-dashed border-purple-400/30 bg-purple-500/[.035] p-8 text-center"
                  >
                    <div>
                      <span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-purple-500/10 text-purple-300">
                        <CloudUpload />
                      </span>
                      <h4 className="mt-4 font-black">
                        {saving
                          ? "Procesando..."
                          : "Arrastra cualquier Excel o CSV"}
                      </h4>
                      <p className="mt-2 text-xs text-zinc-500">
                        Detecta hojas, columnas, cantidades, montos y variantes
                        de nombres.
                      </p>
                      <button
                        type="button"
                        className="mt-5 rounded-xl bg-purple-600 px-5 py-2.5 text-xs font-black"
                      >
                        Seleccionar archivo
                      </button>
                      <input
              ...
              {importRows.length > 0 && (
                <Card className="overflow-hidden">
                  <div className="flex flex-col gap-4 border-b border-white/[.06] p-5 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                      <h3 className="font-black">Vista previa inteligente</h3>
                      <p className="mt-1 text-[10px] text-zinc-600">
                    ...
                    >
                      <Check size={15} /> Confirmar actualización
                    </button>
                  </div>

                  {(unresolvedNames.length > 0 || excludedRows > 0) && (
                    <div className="m-4 space-y-3 rounded-xl border border-amber-500/15 bg-amber-500/[.05] p-4">
                      <p className="text-[10px] leading-5 text-amber-200">
                        <AlertTriangle
                          size={14}
                          className="mr-2 inline"
                        />
                        {unresolvedNames.length} nombres requieren revisión y{" "}
                        {excludedRows} filas quedaron fuera del rango.
                      </p>
                      {unresolvedNames.map((row) => (
                        <label
                          key={row.key}
                          className="grid gap-2 text-[10px] font-bold text-zinc-400 sm:grid-cols-[1fr_1.3fr] sm:items-center"
                        >
                          <span>
                            {row.sale.sellerName}
                            {row.match.ambiguous
                  ...
                  <div className="max-h-[460px] overflow-auto">
                    <table className="w-full min-w-[1100px] text-left text-xs">
                      <thead className="sticky top-0 bg-[#17171e] text-[10px] uppercase tracking-wider text-zinc-600">
                        <tr>
                          <th className="px-5 py-3">Nombre en archivo</th>
                          <th>Vendedor oficial</th>
                          <th>Coincidencia</th>
                          <th>Ventas</th>
                          <th>Fecha</th>
                          <th>Monto total</th>
                      ...
                      <tbody>
                        {importPreview.slice(0, 150).map((row, index) => (
                          <tr
                            key={`${row.sale.sellerName}-${row.sale.saleDate}-${index}`}
                            className="border-t border-white/[.05]"
                          >
                            <td className="px-5 py-3 font-bold text-zinc-200">
                              {row.sale.sellerName}
                            </td>
                            <td>
                              {row.match.seller ? (
                                <span className="text-emerald-300">
                                  {row.match.seller.full_name}
                                </span>
                              ) : (
                            ...
                            <td>
                              ...
                              >
                                ...
                                {Math.round(row.match.score * 100)}%
                              </span>
                            </td>
                            <td className="text-base font-black text-purple-300">
                              {row.units}
                            </td>
                            <td>{formatDate(row.sale.saleDate)}</td>
                            <td>
              ...
              <Card className="overflow-hidden">
                <div className="flex items-center gap-3 border-b border-white/[.06] p-5">
                  <History className="text-purple-300" size={18} />
                  <div>
                    <h3 className="font-black">Historial de cortes cargados</h3>
                    <p className="mt-1 text-[10px] text-zinc-600">
                      Permite saber cuántas ventas llevaba el equipo en cada
                      fecha de corte.
                    </p>
                  </div>
                </div>
                  ...
                  <table className="w-full min-w-[900px] text-left text-xs">
                    ...
                    <tbody>
                      {filteredImports.map((item) => (
                        ...
                        >
                          <td className="px-5 py-3">
                            <p className="max-w-64 truncate font-bold text-zinc-200">
                              {item.file_name}
                            </p>
                          ...
                          <td>{formatDate(item.snapshot_as_of)}</td>
                          <td className="text-base font-black text-purple-300">
                            {item.row_count}
                          </td>
                          <td>
                            {item.import_mode === "replace"
                              ? "SUSTITUCIÓN"
                              : "ACUMULAR"}
                          </td>
                          <td>{item.replaced_rows}</td>
                          <td>
                            {item.superseded_by ? (
                              <span className="text-zinc-600">REEMPLAZADO</span>
                            ) : (
                              <span className="text-emerald-300">VIGENTE</span>
                            )}
                          </td>
                        </tr>
                      ))}
          ...
          {tab === "manual" && (
            <Card className="mx-auto max-w-3xl p-6">
              <div className="flex items-center gap-3">
                <span className="grid h-11 w-11 place-items-center rounded-xl bg-emerald-500/10 text-emerald-300">
                  <Plus />
                </span>
                <div>
                  <h3 className="font-black">Registrar venta por vendedor</h3>
                  <p className="mt-1 text-[10px] text-zinc-600">
                    Las ventas manuales no se eliminan al sustituir un archivo.
                  </p>
                </div>
              </div>
              <form
                onSubmit={saveManualSale}
              ...
              >
                ...
                <label className="text-[10px] font-black uppercase tracking-wider text-zinc-600 sm:col-span-2">
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      setManualSellerId(event.target.value)
                    }
                    className="mt-2 w-full rounded-xl border border-white/[.08] bg-[#111116] p-3 text-xs"
                  >
            ...
            <div className="grid gap-4 xl:grid-cols-[420px_1fr]">
              <Card className="p-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-black">Nueva venta anunciada</h3>
                    <p className="mt-1 text-[10px] text-zinc-600">
                      Aún no cuenta como venta posteada.
                    </p>
                  </div>
                </div>
                <form
                  onSubmit={saveAnnouncedSale}
                ...
                >
                  ...
                  <label className="block text-[10px] font-black uppercase tracking-wider text-zinc-600">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setAnnouncedSellerId(event.target.value)
                      }
                      className="mt-2 w-full rounded-xl border border-white/[.08] bg-[#111116] p-3 text-xs"
                    >
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <label className="text-[10px] font-black uppercase tracking-wider text-zinc-600">
                      ...
                      <input
                        ...
                        value={announcedDate}
                        onChange={(event) =>
                          setAnnouncedDate(event.target.value)
                        }
                        className="mt-2 w-full rounded-xl border border-white/[.08] bg-[#111116] p-3 text-xs"
                      />
                    </label>
                    <label className="text-[10px] font-black uppercase tracking-wider text-zinc-600">
                      ...
                      <input
                        ...
                        step="0.01"
                        min="0"
                        value={announcedAmount}
                        onChange={(event) =>
                          setAnnouncedAmount(event.target.value)
                        }
                        className="mt-2 w-full rounded-xl border border-white/[.08] bg-[#111116] p-3 text-xs"
                      />
                    </label>
                  ...
                  <label className="block text-[10px] font-black uppercase tracking-wider text-zinc-600">
                    ...
                    <input
                      type="date"
                      value={announcedExpected}
                      onChange={(event) =>
                        setAnnouncedExpected(event.target.value)
                      }
                      className="mt-2 w-full rounded-xl border border-white/[.08] bg-[#111116] p-3 text-xs"
                    />
                  </label>
                  <label className="block text-[10px] font-black uppercase tracking-wider text-zinc-600">
                    Observación
                    <textarea
                      value={announcedNotes}
                      onChange={(event) =>
                        setAnnouncedNotes(event.target.value)
                      }
                      className="mt-2 min-h-24 w-full rounded-xl border border-white/[.08] bg-[#111116] p-3 text-xs"
                    />
                  </label>
                  <button
                    ...
                    <tbody>
                      {filteredAnnounced.map((sale) => (
                        ...
                        >
                          ...
                          <td>
                            {new Date(sale.announced_at).toLocaleString("es-HN")}
                          </td>
                          <td>{saleUnits(sale.sale_units)}</td>
                          <td>{money(amount(sale.amount_announced))}</td>
                          <td>{sale.status.toUpperCase()}</td>
                          <td className="pr-5 text-right">
                            {sale.status === "anunciada" && (
                              <div className="flex justify-end gap-2">
                                <button
                                  disabled={saving}
                                  onClick={() =>
                                    void updateAnnounced(sale.id, "posteada")
                                  }
                                  className="rounded-lg bg-emerald-500/10 px-3 py-2 text-[10px] font-black text-emerald-300"
                                >
                                  <Check
                                    size={13}
                                ...
                                <button
                                  disabled={saving}
                                  onClick={() =>
                                    void updateAnnounced(sale.id, "cancelada")
                                  }
                                  className="rounded-lg bg-rose-500/10 px-3 py-2 text-[10px] font-black text-rose-300"
                                >
                      ...
                      {!filteredAnnounced.length && (
                        <tr>
                          <td
                            ...
                            className="px-5 py-8 text-center text-zinc-600"
                          >
                            No hay ventas anunciadas este mes.
                          </td>
                        </tr>
                      )}
                    </tbody>
          ...
          {tab === "report" && (
            <Card className="overflow-hidden">
              <div className="flex flex-col gap-4 border-b border-white/[.06] p-5 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <h3 className="font-black">
                ...
                <table className="w-full min-w-[1100px] text-left text-xs">
                  <thead className="bg-white/[.02] text-[10px] uppercase tracking-wider text-zinc-600">
                    <tr>
                      ...
                      <th>Posteadas</th>
                      <th>Anunciadas</th>
                      <th>Ventas en prueba</th>
                      <th>Monto posteado</th>
                    </tr>
                  </thead>
                  <tbody>
                    {reportRows.map((row, index) => (
                      ...
                      >
                        <td className="px-5 py-3">
                          <p className="font-bold text-zinc-200">
                            {index + 1}. {row.seller.full_name}
                          </p>
                          <p className="mt-1 text-[10px] text-zinc-600">
                            {row.seller.seller_code || "Sin código"}
                          </p>
                        </td>
                        <td>{formatDate(row.seller.hire_date)}</td>
                        <td>{formatDate(row.seller.probation_end_date)}</td>
                        <td>
                          {row.seller.is_on_probation
                            ? "EN PRUEBA"
                            : row.seller.effective_status
                                .replace("_", " ")
                                .toUpperCase()}
                        </td>
                        <td className="text-base font-black text-emerald-300">
                          {row.posted}
                        </td>
                        <td className="text-base font-black text-amber-300">
                          {row.announced}
                        </td>
                        <td>{row.probationSales}</td>
      ...
      <Card className="flex flex-col gap-3 p-4 text-[10px] leading-5 text-zinc-500 sm:flex-row sm:items-center sm:justify-between">
        <span>
          ...
          />
          Retirar un vendedor nunca elimina su historial.
        </span>
        <span>
          <Clock3 size={14} className="mr-2 inline text-amber-300" />
          Con ventas del mes, la salida queda efectiva al iniciar el siguiente.
        </span>
      </Card>
    </div>
  );
