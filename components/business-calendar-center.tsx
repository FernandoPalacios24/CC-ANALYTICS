...
export function BusinessCalendarCenter({ profile }: { profile: Profile }) {
  ...
  async function save() {
    ...
    if (saveError) setError(saveError.message);
      ...
      setNotice("Calendario guardado correctamente. Este módulo todavía no modifica la presentación ni otros tableros.");
      setSelected(null);
      await load();
    }
    setSaving(false);
  ...
  return (
    <div className="animate-in space-y-4 text-white">
      <section className="flex flex-col justify-between gap-4 rounded-2xl border border-white/[.07] bg-white/[.025] p-5 lg:flex-row lg:items-center">
        <div>
          <p className="text-[10px] font-black uppercase tracking-[.2em] text-purple-300">Planeación operativa independiente</p>
          <h2 className="mt-1 text-xl font-black">Calendario laboral y asuetos</h2>
          <p className="mt-1 text-xs text-zinc-500">Domingos no hábiles por defecto. Las ventas no se consultan ni se modifican desde este módulo.</p>
        </div>
