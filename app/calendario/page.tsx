...
export default function CalendarPage() {
  ...
  useEffect(() => {
    void supabase.auth.getSession().then(async ({ data }) => {
      if (!data.session) {
        setError("Inicia sesión antes de abrir el calendario laboral.");
        return;
      }
      const { data: row, error: profileError } = await supabase
        .from("analytics_profiles")
        .select("id,full_name,role,job_profile,department,zone,status,reports_to")
      ...
      if (!["Administrador", "Líder de departamento"].includes(mapped.role)) {
        setError("Solo Administración y líderes pueden editar el calendario laboral.");
        return;
      }
      setProfile(mapped);
    });
  ...
  if (error) {
    return <main className="grid min-h-screen place-items-center bg-[#08080b] p-6 text-rose-300">{error}</main>;
  }
  if (!profile) {
    return <main className="grid min-h-screen place-items-center bg-[#08080b] text-zinc-500">Cargando calendario...</main>;
  }
  return (
    <main className="min-h-screen bg-[#08080b] p-4 sm:p-6">
